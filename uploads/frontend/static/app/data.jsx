...
const REGION_CODE = {
  "춘천시":"80","원주시":"82","강릉시":"81","동해시":"94","태백시":"95","속초시":"96","삼척시":"97",
  "홍천군":"83","횡성군":"84","영월군":"85","평창군":"86","정선군":"87",
  "철원군":"88","화천군":"89","양구군":"90","인제군":"91","고성군":"92","양양군":"93",
};
...
const INCOME_ITEMS = [
  ...
  { value:"선납/초과입금",  accounting:"선납",     tone:"선납" },
];
...
const STREETS = {
  ...
  "동해시":["천곡동","북평동","송정동","북삼동","동호동"],
  "태백시":["황지동","장성동","철암동","문곡소도동","상장동"],
  "속초시":["교동","조양동","금호동","노학동","청호동"],
  "삼척시":["교동","남양동","정라동","도계읍","원덕읍"],
...
function buildMember(i){
  ...
  const mkPay = (ym, amount, method, item, paidDate, memo) => ({
    ...
    amount, method, paidDate, memo: memo || (method+" 반영"), canceled:false,
  });
...
function paidTotal(m){ return (m.payments||[]).filter(p=>!p.canceled && isArrearsIncome(p.chargeItem)).reduce((s,p)=>s+p.amount,0); }
...
function applyPayment(m, { amount, method="직접수납", chargeItem, paidDate, sourceDepositId=null }){
  ...
  if (!deduct) {
    pays.push({ id:nextPid(), paidForYm:date.slice(0,7), chargeItem:item, accounting:accountingOf(item), amount, method, paidDate:date, memo:method+" 반영", canceled:false, sourceDepositId });
  } else {
    let remain = amount;
    ...
    for (const it of opens){
      ...
      pays.push({ id:nextPid(), paidForYm:it.ym, chargeItem:item, accounting:accountingOf(item), amount:pay, method, paidDate:date, memo:method+" 반영", canceled:false, sourceDepositId });
      remain -= pay;
    ...
    if (remain > 0) {
      ...
      pays.push({ id:nextPid(), paidForYm:date.slice(0,7), chargeItem:"선납/초과입금", accounting:accountingOf("선납/초과입금"), amount:remain, method, paidDate:date, memo:"선납/초과입금", canceled:false, sourceDepositId });
    }
  }
...
// ===== 수납 취소/복구 (삭제 X, 취소 상태로 남김) =====
function cancelPayment(m, paymentId, reason="사용자 취소"){
  const nm = { ...m, arrears:(m.arrears||[]).map(a=>({...a})), payments:(m.payments||[]).map(p=>({...p})) };
  const p = nm.payments.find(x=>x.id===paymentId);
  if(!p || p.canceled) return nm;
  p.canceled = true;
  p.cancelReason = reason || "사용자 취소";
  p.canceledAt = new Date().toISOString().slice(0,10);
  if (isArrearsIncome(p.chargeItem)) {
    nm.arrears.push({ ym:p.paidForYm || BASE_YM, label:"수납취소 복구", item:p.chargeItem, amount:Number(p.amount)||0, paid:false, restoredFrom:p.id });
  }
  return nm;
}

// ===== 통장 붙여넣기/업로드 행 파싱 + 후보 생성 =====
function parseBankText(text){
  ...
  lines.forEach((line, idx)=>{
    ...
    const amountPart = parts.find(x=>/[0-9][0-9,]{2,}/.test(x));
    const amount = Number(String(amountPart||"").replace(/[^0-9-]/g,"")) || 0;
    const datePart = parts.find(x=>/\d{2,4}[.\-\/]\d{1,2}[.\-\/]\d{1,2}/.test(x)) || parts[0];
    const namePart = parts.find(x=>x!==datePart && x!==amountPart && !/입금|출금|송금|이체|NH|농협|신한|국민|카카오|토스|폰/.test(x)) || parts[1] || "";
    out.push({
      id: Date.now()+idx,
      depositDate: String(datePart||BASE_YM+"-20").replace(/[.\/]/g,"-"),
      depositorName: namePart || "미확인",
      amount,
      memo: parts.slice(0,8).join(" / "),
      description: "붙여넣기",
      status:"미매칭",
      candidates:[],
      rawText: raw,
...
function inferBankCandidates(deposit, members){
  const text = [deposit.depositorName, deposit.memo, deposit.rawText].join(" ").replace(/\s/g,"").toLowerCase();
  const amount = Number(deposit.amount)||0;
  const candidates = members.filter(m=>m.status==="정상").map(m=>{
    const out = outstanding(m);
    let score=0; const reasons=[];
    if(text.includes(String(m.name||"").replace(/\s/g,"").toLowerCase())){ score+=60; reasons.push("이름 일치"); }
    const l4 = last4(m.vehicleNo);
    if(l4 && text.includes(l4)){ score+=35; reasons.push("차량번호 뒤4자리 일치"); }
    if(amount>0 && out>0 && Math.abs(amount-out)<=0){ score+=35; reasons.push("금액 일치"); }
    if(m.note && String(m.note).length>1 && text.includes(String(m.note).replace(/\s/g,"").toLowerCase())){ score+=15; reasons.push("입금자명 메모 일치"); }
    if(amount>0 && out>0 && Math.abs(amount-out)<=m.monthlyCharge){ score+=10; reasons.push("금액 근접"); }
    return { id:m.id, name:m.name, vehicleNo:m.vehicleNo, mgmtNo:m.mgmtNo, sigun:m.sigun, totalArrears:out, reason:reasons.join(" · ")||"수동확인 필요", reasons, diff:amount-out, score, chargeItem:m.chargeItem };
  }).filter(c=>c.score>=35).sort((a,b)=>b.score-a.score).slice(0,5);
  let status = "미매칭";
  if(candidates.length===1 && candidates[0].score>=90) status="자동매칭";
  else if(candidates.length===1) status="후보확인";
  else if(candidates.length>1) status="중복후보";
  return { ...deposit, candidates, status };
}
...
function buildClosures(members){
  return members.filter(m=>m.status!=="정상").map((m,i)=>({
    id:i+1, memberId:m.id, name:m.name, sigun:m.sigun, vehicleNo:m.vehicleNo, mgmtNo:m.mgmtNo, phone:m.phone, address:m.address,
    ...
    unpaidBalance: outstanding(m), notifyLater: outstanding(m)>0, chaseStatus: outstanding(m)>0?"추심대상":"정산완료", lastNoticeDate:"", chaseMemo:"",
  }));
}
...
function aggregate(list){
  ...
  const thisMonthCollected = list.reduce((s,m)=> s + (m.payments||[]).filter(p=>!p.canceled && String(p.paidDate||"").startsWith("2026-06")).reduce((a,p)=>a+p.amount,0), 0);
...
window.PMData = {
  ...
  basisYm, billingBasisDate, payStatus, applyPayment, cancelPayment, parseBankText, inferBankCandidates, normVehicle, last4,
