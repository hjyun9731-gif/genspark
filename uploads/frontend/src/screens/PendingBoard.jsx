...
export default function PendingBoard({data,addPending,updatePending,deletePending,promotePending}){
  ...
  return <div className="compact-page"><PageHead title="신규 · 예정자" desc="자격증명 발급 → 예정자 등록 → 전체자명단 전환 → 부과시작일 도래 후 미수금 관리 흐름입니다."/>
    <Card className="notice compact-notice" style={{marginBottom:12}}>예정자는 정식 회원이 아닙니다. 전체자명단 전환 후에도 바로 미수금명단에 뜨는 것이 아니라, 부과시작일 이후 미수금이 발생해야 미수금명단 MAIN에 표시됩니다.</Card>
    <Card className="card-pad" style={{marginBottom:12}}><div className="flow"><div>① 자격증명 발급</div><div>② 예정자 등록</div><div>③ 관리번호 부여</div><div>④ 전체자명단 전환</div><div>⑤ 부과시작일 도래</div><div>⑥ 미수금명단 유입</div></div></Card>
    <div className="grid grid-2"><Card className="card-pad"><h3>예정자 등록</h3>
      <div className="form-row"><b>이름</b><input className="input" value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></div>
      <div className="form-row"><b>차량번호</b><input className="input" value={form.vehicleNo} onChange={e=>setForm({...form,vehicleNo:e.target.value})}/></div>
      <div className="form-row"><b>연락처</b><input className="input" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} placeholder="010-0000-0000"/></div>
      <div className="form-row"><b>주소</b><input className="input" value={form.address} onChange={e=>setForm({...form,address:e.target.value})} placeholder="주소 입력"/></div>
      <div className="form-row"><b>지역</b><select className="select" value={form.sigun} onChange={e=>setForm({...form,sigun:e.target.value})}>{SIGUN.map(s=><option key={s}>{s}</option>)}</select></div>
      <div className="form-row"><b>구분</b><select className="select" value={form.memberType} onChange={e=>setForm({...form,memberType:e.target.value})}><option>개인</option><option>택배</option></select></div>
      <div className="form-row"><b>가입여부</b><select className="select" value={form.membership} onChange={e=>setForm({...form,membership:e.target.value})}><option>협회가입</option><option>협회미가입</option></select></div>
      <div className="form-row"><b>발급일자</b><input className="input" type="date" value={form.certIssueDate} onChange={e=>setForm({...form,certIssueDate:e.target.value})}/></div>
      <div className="form-row"><b>비고</b><input className="input" value={form.note} onChange={e=>setForm({...form,note:e.target.value})}/></div>
      <div className="notice">예상: {chargeItem(form.membership)} {monthlyCharge(form.membership).toLocaleString()}원 / 부과시작일 {startDate}<br/>예: 6월 10일 발급이면 7월 10일부터 부과됩니다.</div>
      <button className="btn primary" style={{marginTop:12}} onClick={submit}>예정자 추가</button>
    </Card><Card><h3 style={{padding:'16px 16px 0'}}>예정자 목록</h3><div className="admin-table-wrap"><table className="admin-table dense" style={{minWidth:900}}><thead><tr><th>이름</th><th>차량번호</th><th>지역</th><th>연락처</th><th>주소</th><th>가입</th><th>발급일</th><th>부과시작일</th><th>단계</th><th className="right">처리</th></tr></thead><tbody>{data.pending.map(p=>{const note=p.note||''; const addr=getPart(note,'주소'); const start=getPart(note,'부과시작일') || nextMonthSameDay(p.certIssueDate); return <tr key={p.id}><td><b>{p.name}</b></td><td className="mono nowrap">{p.vehicleNo}</td><td>{p.sigun}</td><td className="mono nowrap">{p.phone||'-'}</td><td className="clip-cell" title={addr}>{addr||'-'}</td><td>{p.membership}</td><td>{p.certIssueDate}</td><td className="mono nowrap">{start}</td><td><Badge tone={p.step==='전환완료'?'green':'blue'}>{p.step}</Badge></td><td className="right action-cell"><button className="btn mini" onClick={()=>setEdit(p)}>수정</button><button className="btn mini red" onClick={()=>{if(confirm('예정자를 삭제할까요?')) deletePending(p.id)}}>삭제</button><button className="btn mini green" disabled={p.step==='전환완료'} onClick={()=>setPromote(p)}>전환</button></td></tr>})}{!data.pending.length&&<tr><td colSpan="10" className="empty-cell compact">등록된 예정자가 없습니다.</td></tr>}</tbody></table></div></Card></div>
