...
function Metric({ icon, tint, label, value, unit, sub, accent, onClick }){
  const [hover,setHover]=React.useState(false);
  return (
    <div onClick={onClick} onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
      style={{ background:"var(--white)", border:"1px solid var(--border-subtle)", borderRadius:"var(--radius-lg)", padding:"18px 20px",
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <span style={{ width:34, height:34, borderRadius:9, background:tint||"var(--brand-subtle)", display:"inline-flex", alignItems:"center", justifyContent:"center", flex:"none" }}>
          <Icon name={icon} size={18} color={accent||"var(--brand)"} /></span>
        {onClick && <Icon name="chevron-right" size={14} style={{ color:"var(--text-tertiary)" }} />}
      </div>
      <div style={{ font:"var(--fw-medium) 13px/1.4 var(--font-sans)", color:"var(--text-secondary)", marginTop:14 }}>{label}</div>
      <div style={{ display:"flex", alignItems:"baseline", gap:4, marginTop:4 }}>
        <span style={{ font:"var(--fw-bold) 26px/1.15 var(--font-sans)", color:"var(--text-primary)", letterSpacing:"-0.02em" }}>{value}</span>
        {unit && <span style={{ font:"var(--fw-medium) 14px/1 var(--font-sans)", color:"var(--text-tertiary)" }}>{unit}</span>}
      </div>
      {sub && <div style={{ font:"var(--body-xs)", color:"var(--text-tertiary)", marginTop:6 }}>{sub}</div>}
    </div>
  );
}
...
function Dashboard({ agg, members, deposits, closures, onDrill, onNav, year, month }){
  ...
  const maxBucket = Math.max(...agg.buckets.map(b=>b.count), 1);
