...
function Regional({ members, onToast }){
  ...
  const filtered = React.useMemo(()=>{
    return members.filter(m=>{
      ...
      if (isSms){
        ...
        if (onlyUnpaid && out<=0) return false;
  ...
  }, [members,regions,charges,senior,incZero,incPrepaid,onlyUnpaid,minAmt,isSms,excludeNoPhone,excludeDisconnected,excludeAutoPay]);
  ...
  const prepaidTotal = filtered.reduce((s,m)=>s+Math.min(D.outstanding(m),0),0);
  const zeroCount = filtered.filter(m=>D.outstanding(m)===0).length;
  const smsCount = filtered.filter(m=>m.phone && !m.disconnected && D.outstanding(m)>0).length;

  function copyPhones(){
    const phones = filtered.map(m=>m.phone).filter(Boolean).join("\n");
    navigator.clipboard && navigator.clipboard.writeText(phones);
    onToast(`전화번호 ${phones?phones.split("\n").length:0}건 복사 완료`);
  }
  function copyMessages(){
    const msgs = filtered.filter(m=>m.phone).map(m=>`${m.phone}\t[화물협회] ${m.name}님 미수금 ${won(D.outstanding(m))} 납부 안내드립니다.`).join("\n");
    navigator.clipboard && navigator.clipboard.writeText(msgs);
    onToast(`문자문구 ${msgs?msgs.split("\n").length:0}건 복사 완료`);
  }
  ...
  return (
    <div style={{ display:"grid", gridTemplateColumns:"300px 1fr", gap:24, alignItems:"start" }}>
      ...
      <Card style={{ position:"sticky", top:0 }}>
        ...
        <div style={sectionTitle}>대상조건</div>
        ...
        <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:8 }}>
          {[["전체",0],["50원↑",50],["3만원↑",30000],["20만원↑",200000],["30만원↑",300000]].map(([l,v])=>(
        ...
        {!isSms ? (
          <>
            ...
            <OptToggle label="미납만 보기" checked={onlyUnpaid} onChange={setOnlyUnpaid} />
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:"var(--white)", border:"1px solid var(--border-subtle)", borderRadius:"var(--radius-lg)", padding:"16px 20px", boxShadow:"var(--shadow-xs)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:24 }}>
            ...
            <div style={{ width:1, height:34, background:"var(--border-default)" }} />
            <div><div style={{ font:"var(--body-xs)", color:"var(--text-tertiary)" }}>선납 합계</div><div style={{ font:"var(--fw-bold) 22px/1.1 var(--font-sans)", color:"var(--violet-500)" }}>{won(prepaidTotal)}</div></div>
            <div style={{ width:1, height:34, background:"var(--border-default)" }} />
            <div><div style={{ font:"var(--body-xs)", color:"var(--text-tertiary)" }}>0원 인원</div><div style={{ font:"var(--fw-bold) 22px/1.1 var(--font-sans)", color:"var(--text-primary)" }}>{zeroCount}<span style={{ fontSize:14, color:"var(--text-tertiary)", fontWeight:500 }}>명</span></div></div>
            <div style={{ width:1, height:34, background:"var(--border-default)" }} />
            <div><div style={{ font:"var(--body-xs)", color:"var(--text-tertiary)" }}>{isSms?"문자 가능":"지역 수"}</div><div style={{ font:"var(--fw-bold) 22px/1.1 var(--font-sans)", color:"var(--text-primary)" }}>{isSms?smsCount:groups.length}<span style={{ fontSize:14, color:"var(--text-tertiary)", fontWeight:500 }}>{isSms?"명":"개"}</span></div></div>
          </div>
          <div style={{ display:"flex", gap:8 }}>
            {isSms && <button type="button" onClick={copyPhones} style={{ height:36, padding:"0 12px", borderRadius:"var(--radius-pill)", border:"1px solid var(--border-default)", background:"var(--white)", cursor:"pointer", color:"var(--text-secondary)", font:"var(--fw-medium) 13px/1 var(--font-sans)" }}>전화번호 복사</button>}
            {isSms && <button type="button" onClick={copyMessages} style={{ height:36, padding:"0 12px", borderRadius:"var(--radius-pill)", border:"1px solid var(--border-default)", background:"var(--white)", cursor:"pointer", color:"var(--text-secondary)", font:"var(--fw-medium) 13px/1 var(--font-sans)" }}>문자문구 복사</button>}
          ...
          return (
            <Card key={g.region} padded={false}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 20px", borderBottom:"1px solid var(--border-subtle)", background:"var(--grey-25)", borderTopLeftRadius:"var(--radius-lg)", borderTopRightRadius:"var(--radius-lg)" }}>
                ...
                <span style={{ font:"var(--fw-demibold) 14px/1 var(--font-sans)", color:"var(--text-primary)" }}>미수 {won(sub)} · 선납 {won(g.rows.reduce((s,m)=>s+Math.min(D.outstanding(m),0),0))} · 0원 {g.rows.filter(m=>D.outstanding(m)===0).length}명</span>
              </div>
