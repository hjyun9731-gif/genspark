...
function Receivables({ members, drill, density, onPay, onSelect, onClose, onToast }){
  ...
  const rows = React.useMemo(()=>{
    ...
    let list = members.filter(m=>{
      ...
      if (amount === "20만원이상" && !(out>=200000)) return false;
  ...
  const PAY_TABS = [["전체","전체"],["미수있음","미납"],["완납","완납/0원"],["선납","선납/초과"],["20만원이상","20만원↑"],["30만원이상","30만원↑"]];
  const countByAmount = (key)=> members.filter(m=>{
    ...
    if(key==="20만원이상") return out>=200000;
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ border:"1px solid var(--border-subtle)", borderRadius:"var(--radius-lg)", overflow:"hidden", background:"var(--white)", boxShadow:"var(--shadow-xs)" }}>
                  ...
                  <tr key={m.id} onClick={()=>onSelect(m)}
                    ...
                    <td style={{ padding:pad, whiteSpace:"nowrap" }}>
                      <span style={{ display:"inline-flex", alignItems:"center", gap:8 }}>
                        ...
                        {m.disconnected && <span style={{ font:"10px/1 var(--font-sans)", color:"var(--red-500)", fontWeight:700, padding:"2px 5px", background:"var(--red-50)", borderRadius:4 }}>결번</span>}
                        {String(m.note||"").includes("주소불명") && <span style={{ font:"10px/1 var(--font-sans)", color:"#B9791A", fontWeight:700, padding:"2px 5px", background:"var(--amber-50)", borderRadius:4 }}>주소불명</span>}
                        {m.certMissing && <span style={{ font:"10px/1 var(--font-sans)", color:"var(--violet-500)", fontWeight:700, padding:"2px 5px", background:"#EFEEFD", borderRadius:4 }}>자격미발급</span>}
                        {String(m.note||"").includes("자동이체") && <span style={{ font:"10px/1 var(--font-sans)", color:"var(--green-500)", fontWeight:700, padding:"2px 5px", background:"var(--green-50)", borderRadius:4 }}>자동이체</span>}
                      </span>
                    </td>
                    ...
                    <td style={{ padding:pad, textAlign:"right", font:"var(--body-sm)", color:"var(--green-500)", whiteSpace:"nowrap", fontVariantNumeric:"tabular-nums" }}>{won(D.paidTotal(m))}</td>
                    ...
                    <td style={{ padding:pad, textAlign:"right" }}>
                      <button type="button" onClick={(e)=>{ e.stopPropagation(); onSelect(m); }}
                        style={{ height:30, padding:"0 10px", borderRadius:"var(--radius-pill)", whiteSpace:"nowrap", border:"1px solid var(--border-default)", cursor:"pointer", background:"var(--white)", color:"var(--text-secondary)", font:"var(--fw-demibold) 12px/1 var(--font-sans)", marginRight:6 }}>상세</button>
                      <button type="button" onClick={(e)=>{ e.stopPropagation(); onPay(m); }}
                        style={{ height:30, padding:"0 13px", borderRadius:"var(--radius-pill)", whiteSpace:"nowrap",
                          ...
                          font:"var(--fw-demibold) 12px/1 var(--font-sans)", transition:"all .12s", marginRight:6 }}
                        ...
                        수납</button>
                      <button type="button" onClick={(e)=>{ e.stopPropagation(); navigator.clipboard&&navigator.clipboard.writeText(`${m.name} ${m.phone||""} ${D.won(D.outstanding(m))}`); onToast&&onToast("문자대상 정보 복사"); }}
                        style={{ height:30, padding:"0 10px", borderRadius:"var(--radius-pill)", whiteSpace:"nowrap", border:"1px solid var(--border-default)", cursor:"pointer", background:"var(--white)", color:"var(--text-secondary)", font:"var(--fw-demibold) 12px/1 var(--font-sans)" }}>문자</button>
                    </td>
                  </tr>
                );
              })}
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 18px", borderTop:"1px solid var(--border-subtle)", background:"var(--grey-25)" }}>
          <span style={{ font:"var(--body-xs)", color:"var(--text-tertiary)" }}>이름/상세 버튼 클릭 시 상세 · 0원·선납 회원도 함께 표시 (협회 관리 대상 전체 명단)</span>
